...
use constant PRODUCT    => 'img_date v1.0';
use constant FG         => '#EAEAEA';
use constant BG         => '#4F6D7A';
use constant HBG        => '#869CA5';
use constant HELP       => <<EOT;
img_date -target C:\\tmp\\chosen

-target   \x{2022} Rename folders to match created date
...
sub _get_paths {
    ...
    return ("$vol$path$prog", "$vol$path$prog", "$prog");
}
...
sub _cat {
    ...
    my $funcf   = sub { 
        my $src             = shift;
        my $f               = shift;
        my $image           = new Image::MetaData::JPEG("$src/$f", 'COM', 'FASTREADONLY');
        my $data            = $image->get_Exif_data('IMAGE_DATA', 'TEXTUAL');
        my $ctime           = $data->{DateTimeOriginal}->[0];
        print "$f $ctime\n";
